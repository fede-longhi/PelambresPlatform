---
name: ui-design
description: Designs and reviews PelambresPlatform UI/UX for public, admin, and customer surfaces. Use when the user asks to design a screen, improve UX, review UI consistency, audit responsive layout, or evaluate visual polish of pages and components.
---
...
| **Review** | "review UI", "audit", "check responsive", PR/diff | Markdown review using [review-template.md](review-template.md) |
...
6. **Implement** — Server Components by default; `'use client'` only for hooks/interactivity.

## Review workflow

1. Open the page(s) or diff; compare with 1–2 reference pages on the same surface.
2. Walk the rubric in [review-template.md](review-template.md).
3. Classify findings: **Crítico** (broken UX/accessibility), **Mejora** (inconsistency), **Opcional** (polish).
4. For each finding: location, issue, suggested fix (concrete class/component).
5. If asked to fix: apply minimal diffs; do not redesign unrelated areas.

## Surface quick reference

| Surface | Shell | Typography | Tone |
|---------|-------|------------|------|
| Public | `MainHeader` + `MainFooter`, `max-w-7xl mx-auto` | Marketing headings + `text-primary` accents | Confident, approachable, voseo |
...
- *"Review the login and forgot-password UI for consistency"* → review mode, auth surface, output `review-template.md` format.
